...
export const organizations = pgTable('organizations', {
  ...
  validTo: timestamp('valid_to'), // NULL = aktuell gültig
  isCurrent: boolean('is_current').default(true).notNull(),
  previousVersionId: uuid('previous_version_id').references((): any => organizations.id),
...
export const users = pgTable('users', {
  ...
  isCurrent: boolean('is_current').default(true).notNull(),
  previousVersionId: uuid('previous_version_id').references((): any => users.id),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
});
...
export const roles = pgTable('roles', {
  ...
  validTo: timestamp('valid_to'),
  isCurrent: boolean('is_current').default(true).notNull(),
  previousVersionId: uuid('previous_version_id').references((): any => roles.id),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
});
...
export const staff = pgTable('staff', {
  ...
  validTo: timestamp('valid_to'),
  isCurrent: boolean('is_current').default(true).notNull(),
  previousVersionId: uuid('previous_version_id').references((): any => staff.id),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
});
...
export const staffAddresses = pgTable('staff_addresses', {
  ...
  addressType: varchar('address_type', { length: 50 }).default('primary'), // primary, billing, shipping
  // SCD Type 2 Felder
  validFrom: timestamp('valid_from').defaultNow().notNull(),
  validTo: timestamp('valid_to'),
  isCurrent: boolean('is_current').default(true).notNull(),
  previousVersionId: uuid('previous_version_id').references((): any => staffAddresses.id),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
});
...
export const shifts = pgTable('shifts', {
  ...
  requiredStaffCount: varchar('required_staff_count', { length: 50 }), // z.B. "2 Kellner, 1 Koch"
  notes: text('notes'),
  ...
  validTo: timestamp('valid_to'),
  isCurrent: boolean('is_current').default(true).notNull(),
  previousVersionId: uuid('previous_version_id').references((): any => shifts.id),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
});
...
export const shiftAssignments = pgTable('shift_assignments', {
  ...
  staffId: uuid('staff_id').notNull().references(() => staff.id, { onDelete: 'cascade' }),
  roleId: uuid('role_id').references(() => roles.id, { onDelete: 'set null' }), // In welcher Rolle wird die Person eingeteilt
...
export const shiftAssignmentLogs = pgTable('shift_assignment_logs', {
  ...
  previousStatus: responseStatusEnum('previous_status'),
  newStatus: responseStatusEnum('new_status').notNull(),
  changedBy: uuid('changed_by').references(() => users.id, { onDelete: 'set null' }), // Wer hat die Änderung vorgenommen (Manager oder Staff selbst)
  changedByStaff: uuid('changed_by_staff').references(() => staff.id, { onDelete: 'set null' }), // Falls Staff die Änderung vorgenommen hat
...
export const auditLogs = pgTable('audit_logs', {
  ...
  entityId: uuid('entity_id').notNull(), // ID der betroffenen Entität
  action: auditActionEnum('action').notNull(),
  performedBy: uuid('performed_by').references(() => users.id, { onDelete: 'set null' }),
  performedByStaff: uuid('performed_by_staff').references(() => staff.id, { onDelete: 'set null' }),
  oldValues: json('old_values'), // Vorherige Werte (JSON)
  newValues: json('new_values'), // Neue Werte (JSON)
  changedFields: json('changed_fields').$type<string[]>(), // Array der geänderten Felder
  ipAddress: varchar('ip_address', { length: 45 }), // IPv4 oder IPv6
...
export const shiftAssignmentsRelations = relations(shiftAssignments, ({ one, many }) => ({
  ...
  role: one(roles, {
    fields: [shiftAssignments.roleId],
    references: [roles.id],
  }),
...
export const shiftAssignmentLogsRelations = relations(shiftAssignmentLogs, ({ one }) => ({
  assignment: one(shiftAssignments, {
    fields: [shiftAssignmentLogs.assignmentId],
    references: [shiftAssignments.id],
  }),
  changedByUser: one(users, {
    fields: [shiftAssignmentLogs.changedBy],
    references: [users.id],
  }),
  changedByStaffMember: one(staff, {
    fields: [shiftAssignmentLogs.changedByStaff],
    references: [staff.id],
  }),
}));
